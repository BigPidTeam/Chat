...
columns = ["ID", "카테고리_ID", "카테고리명", "제목", 
               "등록날짜", "거래구분", "판매금액", "제품설명", 
               "지불방법", "배송방법", "상세설명", "수집날짜"]
external_df = pd.DataFrame(columns=columns)
...
for i in range(1,13):
    ...
    frames = [external_df, df]
    external_df = pd.concat(frames)
    external_df.to_csv("newdata.csv", encoding='utf-8', columns=columns, index=False)
